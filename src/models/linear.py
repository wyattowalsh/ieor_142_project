...
def baseline_creation(save = False):
	...
	for i, file_name in enumerate(file_names):
		...
		model = linear(sets[file_name][0],sets[file_name][2])
		preds = model.predict(sets[file_name][1])
		ols_score = metrics.apply_metrics('{} OLS'.format(names[i]), sets[file_name][3], preds)
...
def collect_statistics(name, X_train, X_test, y_train, y_test):
	...
	for i,model in enumerate(models):
		...
		if (name == 'dataset_1' and model_name == 'OLS') \
		or (name == 'dataset_2' and model_name == 'OLS') \
		or (name == 'dataset_3' and model_name == 'OLS'):
			continue
		for j, variation in enumerate(variation_strings):
			model_metrics = metrics.apply_metrics('{} {}{}'.format(display_name, model_name, variation), 
	...
	for i, model in enumerate(models[1:]):
		...
		if model_name == 'OLS':
			continue
		for j, variation in enumerate(variation_strings):
			model_metrics = metrics.apply_metrics('{} {}{} TimeSeriesSplit'.format(display_name, model_name, variation), 
			                                      y_test, built_models[j].predict(variation_test[j]))
			regression_statistics = pd.concat([regression_statistics, model_metrics], axis = 0)
...
def huber(X_train, y_train, cv = 5):
	...
	param_grid = {'epsilon': np.linspace(1, 2, 12),
	'alpha': np.linspace(1e-6, 1e6+1, 12)}
