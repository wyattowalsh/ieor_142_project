...
def adaboost_randomized_cv(X_train, y_train, n_iter = 200, cv = 5):
	...
	y_train = y_train.copy()
	to_score = metrics.create_metrics()[0]
	param_grid = {'base_estimator': 5,
				'n_estimators': np.linspace(20, 2000, 10, dtype=int),
				  'learning_rate': np.linspace(0.01, 1, 10)} 
	adaboost = AdaBoostRegressor(n_jobs=-1, random_state = 18)
	adaboost_cv = RandomizedSearchCV(estimator=adaboost, param_distributions = param_grid, n_iter = n_iter, 
									 cv=cv, refit='Mean Absolute Error', iid=False, 
...
def gradient_boosting_randomized_cv(X_train, y_train, n_iter = 200, cv = 5):
	...
	gradient_boosting = GradientBoostingRegressor(n_jobs = -1, random_state = 18)
	gradient_boosting_cv = RandomizedSearchCV(estimator= gradient_boosting, param_distributions = param_grid,
											n_iter = n_iter, cv=cv, refit='Mean Absolute Error', iid=False, 
											scoring = to_score).fit(X_train, y_train)

	return gradient_boosting_cv.cv_results_
...
def extra_trees_randomized_cv(X_train, y_train, n_iter = 200, cv = 5):
	...
	extra_trees_cv = RandomizedSearchCV(estimator=extra_trees, param_distributions=random_grid,
										n_iter=n_iter, cv=cv, refit='Mean Absolute Error',
										iid=False, scoring = to_score).fit(X_train, y_train)
...
def extra_trees_grid_cv(X_train,y_train, cv = 5):
	...
	extra_trees_cv = GridSearchCV(estimator= extra_trees, param_grid = param_grid,
